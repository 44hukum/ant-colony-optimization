...
foreach (Connection aConnection in ConnectionsFromNodeAndNotVisited)
                    ...
                    // Travel down the path with the largest probability - or have a random choice if
/*                    there are paths with equal probabilities.*/
// Loop through the paths and check if visited destination already.
...
private List<Connection> AllConnectionsFromNodeAndNotVisited(GameObject FromNode, List<Connection>
Connections, List<GameObject> VisitedList)
{
    List<Connection> ConnectionsFromNode = new List<Connection>();
    foreach (Connection aConnection in Connections)
    {
...
// Sum the product of the pheromone level and the visibility factor on all allowed paths.
private float CalculateTotalPheromoneAndVisibility(List<Connection>
ConnectionsFromNodeAndNotVisited)
{
    ...
    foreach (Connection aConnection in ConnectionsFromNodeAndNotVisited)
    {
        TotalPheromoneAndVisibility += (Mathf.Pow(aConnection.GetPheromoneLevel(), Alpha) *
        Mathf.Pow((1 / aConnection.GetDistance()), Beta));
    }
    return TotalPheromoneAndVisibility;
}
public List<Connection> GenerateRoute(GameObject StartNode, int MaxPath, List<Connection>
Connections)
{
    ...
    while (CurrentNode != null)
    {
        ...
        // If the current node is the start node at this point then we have looped through the pathand should stop.
...
EvaporationFactor + ", DefaultPheromone: " +
DefaultPheromone + "):\n";
    int PathCount = 1;
    while (CurrentNode != null)
    {
...
"):\n";
    ...
    {
        Output += "Ant " + i + " - Start Node: " + Ants[i].GetStartNode().name + " | Tour Length: " +
        Ants[i].GetAntTourLength() + "\n";
    }
    Debug.Log(Output);
}
}